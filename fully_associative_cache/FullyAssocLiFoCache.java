...
class FullyAssocLiFoCache implements Cache {
	...
	FullyAssocLiFoCache(int cacheSize, int cacheLineSize) {
		...
		// WV: Your initializations here
		cache_storage = new int[CACHE_SZ];
		location_stack = new Stack<Integer>();

		// pushing locations of empty cache lines going from the last to the first one
	...
	public void flush(int[] ram, Status status) {
		...
		// WV: Your other data structures here
		// make a copy of the cache_loc_to_address and write to memory using the copied
		// hashmap
		HashMap<Integer, Integer> copy = new HashMap<Integer, Integer>(cache_loc_to_address);

		for (int key : copy.keySet()) {
			write_to_mem_on_evict(ram, key);
		}

		// reset everything as it was in the constructor
		cache_storage = new int[CACHE_SZ];
		last_used_loc = CACHELINE_SZ - 1;
	...
	private void write_data_to_cache(int[] ram, int address, int data, Status status) {
		...
		// ...
		status.setHitOrMiss(true); // assume there is a cache hit and check if it is true in if statement

		if (!address_in_cache_line(address)) {
			status.setHitOrMiss(false); // i.e. miss

			// if cache is full, save the data from the last used location to the DRAM and
			// empty that location
			if (cache_is_full()) {
			...
			// if it was a cache miss, read from the DRAM
	...
	private void read_from_mem_on_miss(int[] ram, int address) {
		...
		// populate the array cache_line by taking entries from the DRAM
		for (int i = 0; i < CACHELINE_SZ; i++) {
			cache_line[i] = ram[cache_line_start_mem_address(address) + i];
		}

		address_to_cache_loc.put(address, loc);
		cache_loc_to_address.put(loc, address);

		// populate cache_storage with entries from the cache_line
		for (int i = 0; i < CACHELINE_SZ; i++) {
	...
	private int fetch_cache_entry(int address) {
		...
		loc = address_to_cache_loc.get(cache_line_address(address));
		last_used_loc = loc;
		return cache_storage[loc + cache_entry_position(address)];
	}
	...
	private void evict_location(int loc) {
		...
		address_to_cache_loc.remove(cache_loc_to_address.remove(loc));
	}
	...
	private void write_to_mem_on_evict(int[] ram, int loc) {
		...
		int ram_address = cache_line_start_mem_address(evicted_cl_address);

		for (int i = 0; i < CACHELINE_SZ; i++) {
			ram[ram_address + i] = cache_storage[loc + i];
		}
	...
	private boolean address_in_cache_line(int address) {
		...
		// ...
		address = cache_line_address(address);
		return address_to_cache_loc.containsKey((Integer) address);
	}
